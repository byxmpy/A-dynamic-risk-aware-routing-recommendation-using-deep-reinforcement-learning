...
import graph
import numpy as np
origin_distance=np.load('80distance10.npy')
distance = origin_distance
origin_crime=np.load('80crime10.npy')
crime = origin_crime
...
order_price_dist = []
mapped_matrix_int = np.array([[5, 8, 2, 17, 15, 26, 27, 25, 30, 28], [1, 7, 3, 23, 19, 29, 31, 33, 35, 37], [6, 4, 9, 11, 22, 32, 34, 36, 38, 39],
 [16, 10, 13, -100, 21, 40, 42, 44, 46, 48], [12, 14, 18, 20, 24, 41, 43, 45, 47, 49], [50, 52, 54, 56, 58, 60, 62, 64, 66, 68], 
 [51, 53, 55, 57, 59, 61, 63, 65, 67, 69], [70, 72, 74, 76, 78, 80, 82, 84, 86, 88], [71, 73, 75, 77, 79, 81, 83, 85, 87, 89],[90, 92, 94, 96, 98, 91, 93, 95, 97, 99]])
M, N = mapped_matrix_int.shape
order_num_dist = []
num_valid_grid = 99
...
for n_iter in np.arange(180):
    ...
    for ii in np.arange(EP_LEN + 1):
        ...
        for node in action_tuple:
            ...
            cost2 = (find_du(start,daoda,10)-find_du(end,daoda,10))*cost2_parameter
            ...
            # if crime[start][end] >= first_crime + 20:
            #     crime[start][end] = first_crime + 20
            # if crime[start][end] <= first_crime*0.88:  #防止少的太多吧
            #     crime[start][end] = first_crime*0.88
            # if crime[start][end] <= 1 and crime[start][end] > 0:
            #     crime[start][end] = 1
            ...
            if rl_end == 0:
                if ii >= 30:
                    ...
                    if start != start_node and number_max > 0 and flag != 3:#判断有路线而且过了出发点判定
                        ...
                        flag = 3
                        if next_node == daoda:
                            reward_cost[end] += daoda_parameter1 + daoda_parameter2*find_du(chufa,daoda,10)
                            rl_end = 1
                            rl_end_time = tttt()
...
data.to_excel('80risk_10_data1.00_0.08_29.xls')
