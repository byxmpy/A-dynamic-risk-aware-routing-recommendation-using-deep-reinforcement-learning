...
import graph
import numpy as np
distance=np.load('80distance10.npy')
crime=np.load('80crime10.npy')
...
for n_iter in np.arange(180):
    ...
    for ii in np.arange(EP_LEN + 1):
        ...
        risk_change_parameter1 = 1.35
        risk_change_parameter2 = 0.2
        ...
        if rl_end == 1:#如果RL已经结束了但是A*和迪杰斯特拉没结束
            if axing_i < len(axing)-1:
                a_distance_cost += distance[axing[axing_i]][axing[axing_i+1]]
                a_crime_cost += crime[axing[axing_i]][axing[axing_i+1]]
                a_distance_single_cost.append(distance[axing[axing_i]][axing[axing_i+1]])
...
data.to_excel('80risk_10_data13510_180_5.xls')

from multiprocessing import Pool
def main(name, num):
    print "{}{}Hello World".format(num,name)
