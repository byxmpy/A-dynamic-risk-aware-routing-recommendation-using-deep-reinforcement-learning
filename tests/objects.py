...
class Node(object):
            ...
            for key, assigned_driver in self.drivers.iteritems():
                if assigned_driver.onservice is False and assigned_driver.online is True:
                    if order_to_serve.get_end_position() is not None:
                        ...
                        assigned_driver.set_offline()  # order destination is not in target region  应该是跑到地图外面去了
                    city.n_drivers -= 1 #城市的空闲司机数量-1
                    break

        all_order_num = len(self.orders)    #orders是储存的待接单的订单
        finished_order_num = len(served_order_index)#这是应该是本次分派订单中被分配的订单数量，为什么取finished名字呢？
    ...
    def simple_order_assign_broadcast_update(self, city, neighbor_node_reward):
        ...
        for neighbor_node in self.neighbors:
            if neighbor_node is not None and neighbor_node.idle_driver_num > 0:#挑选有闲置司机的结点
