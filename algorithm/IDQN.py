...
class Estimator:
        ...
        for grid_idx, grid_id in enumerate(env.target_grids):
            for neighbor_idx, neighbor in enumerate(self.env.nodes[grid_id].neighbors):
                if neighbor is None:
                    ...
                    #print node_index, self.env.nodes[grid_id].neighbors
    ...
    def action(self, s, context, epsilon):
        ...
        # print "len(self.env.nodes"
        # print len(self.env.nodes)
        action_tuple = []
        action_starting_gridids = []
        # np.eye(self.n_valid_grids)
        grid_ids = np.argmax(s[:, -self.n_valid_grid:], axis=1)  # starting grid of each sample返回最后n行每行最大值的索引
        valid_probs = []
        ##print "grid_ids"
        ##print grid_ids
        ...
        for idx, grid_valid_idx in enumerate(grid_ids):
            ...
            ##print "curr_qvalue {} ".format(curr_qvalue)
            ##print "temp_qvalue {} ".format(temp_qvalue)

            if np.sum(temp_qvalue) < 0:  # encourage exploration       这里有些不对劲，怎么可能碰巧是0的时候，应该是<=0，执行积极的探索

                temp_qvalue[self.valid_action_mask[grid_valid_idx] > 0] = 1. / np.sum(                  # 这里把每个有效的action值(>0)都设置成一样的了
                ...
                best_action = np.argmax(temp_qvalue)    # 找到最大的reward的动作
                action_prob = np.zeros(self.action_dim)
                num_valid_action = np.count_nonzero(temp_qvalue)    # 合法的动作空间个数(如果是有一个action的curr_qvalue正好是0呢，也会被误认为是invalid)
                # ziji
                for idz, idy in enumerate(self.valid_neighbor_node_id[idx]):
                    if idy == -1:
                        temp_qvalue[idz] = -100
            ...
            i = 0
            # print "temp_qvalue {} ".format(temp_qvalue)
            ##print "curr_action_indices{} ".format(curr_action_indices)
            # print "valid_neighbor_node_id{} ".format(self.valid_neighbor_node_id)
            for curr_action_idx, num_driver in enumerate(curr_action_indices):
                if num_driver > 0:
                    ...
                    # print "i{}   grid_valid_idx{}   curr_action_idx {}   end_node_id{}     action_idx_valid{}".format(i,grid_valid_idx,curr_action_idx,end_node_id,action_idx_valid)
                    i += 1
            action_starting_gridids.append(num_distinct_action)

        action_indices = np.where(action_tuple_mat > 0)

        # print "{}action_indices   action_tuple_mat {} ".format(action_indices, action_tuple_mat)
...
class stateProcessor:
    ...
    def to_grid_states(self, curr_s, curr_city_time):
        ...
        onehot_grid_id = np.eye(self.n_valid_grids) # NxN的对角矩阵，N为n_valid_grids
        # print onehot_grid_id
        s_grid = np.zeros((self.n_valid_grids, self.n_valid_grids * 3 + T))     # example中就是 8*(144+8*3)=8*168
        # print s_grid.shape
    ...
    def to_grid_next_states(self, s_grid, next_state, action_index, curr_city_time):
       ...
       # print "next_s {} , s_grid {}".format(next_s.shape, s_grid.shape)
